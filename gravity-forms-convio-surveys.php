...
class GFConvio {
    ...
    public static function tooltips($tooltips){
        $convio_tooltips = array(
            ...
            "convio_gravity_form" => "<h6>" . __("Gravity Form", "gravity-forms-convio") . "</h6>" . __("Select the Gravity Form you would like to integrate with Convio. Contacts generated by this form will be automatically added to your Convio account.", "gravity-forms-convio"),
            ...
            "convio_map_fields" => "<h6>" . __("Map Fields", "gravity-forms-convio") . "</h6>" . __("Associate your Convio merge variables to the appropriate Gravity Form fields by selecting.", "gravity-forms-convio"),
    ...
    public static function settings_page(){
        ...
        <form action="" method="post">
            ...
            <?php if(GFCommon::current_user_can_any("gravityforms_convio_uninstall")){ ?>
                ...
                <div class="delete-alert"><?php _e("Warning! This operation deletes ALL Convio Feeds.", "gravity-forms-convio") ?>
                    <?php
                    $uninstall_button = '<input type="submit" name="uninstall" value="' . __("Uninstall Convio Add-On", "gravity-forms-convio") . '" class="button" onclick="return confirm(\'' . __("Warning! ALL Convio Feeds will be deleted. This cannot be undone. \'OK\' to delete, \'Cancel\' to stop", "gravity-forms-convio") . '\');"/>';
    ...
    private static function list_page(){
        ...
        <div class="wrap">
            ...
            <h2><?php _e("Convio Feeds", "gravity-forms-convio"); ?>
                        ...
                        if(is_array($settings) && sizeof($settings) > 0){
                            ...
                            <tr>
                                <td colspan="4" style="padding:20px;">
                                    <?php _e(sprintf("You don't have any Convio feeds configured. Let's go %screate one%s!", '<a href="admin.php?page=gf_convio&view=edit&id=0">', "</a>"), "gravity-forms-convio"); ?>
                                </td>
                            </tr>
                            <?php
                        }
    ...
    private static function is_valid_login($settings){
		...
		$convioAPI->short_name      = $shortname;
        $convioAPI->login_name      = $username;
        $convioAPI->login_password  = $password;

		// Get Auth token
		$auth = $convioAPI->call('SRConsAPI_getSingleSignOnToken');
    ...
    public static function select_convio_form(){
        ...
        //getting list of all Convio merge variables for the selected contact list
            ...
            switch(strtolower($field_id))
            {
            	...
            	default :
		            ...
		            if($field_id == intval($field_id) && RGFormsModel::get_input_type($field) == "address") //handling full address
		                ...
		                $merge_vars[$var_tag] = apply_filters("gform_convio_field_value", rgar($entry, $field_id), $form["id"], $field_id, $entry);
            		break;
            }
